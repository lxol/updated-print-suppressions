...
class RandomDataGenerator extends UnitSpec {
  ...
  "RandomDataGenerator" should {
    ...
    "create 3M random records in one day" in new TestSetup {
      ...
      val repository: UpdatedPrintSuppressionsRepository = new UpdatedPrintSuppressionsRepository(new LocalDate().minusDays(1), name => new MongoCounterRepository(name))
